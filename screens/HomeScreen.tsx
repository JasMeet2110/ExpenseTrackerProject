...
export default function HomeScreen() {
  ...
  const progressPercent = Math.min((expenses / Number(income)) * 100, 100).toFixed(0) + '%';
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.summary}>
          ...
          <View style={styles.progressBar}>
            <View style={[styles.progressFill, { width: `${(expenses / Number(income)) * 100}%` }]} />
          </View>
          <Text style={styles.remainingLabel}>Remaining: ${remaining} ({progressPercent} used)</Text>
        </View>
        ...
        <View style={styles.transactions}>
          ...
          {[
            { category: '(No expenses yet! Click to add.)', amount: 0 },
          ].map((tx, idx) => (
            <View key={idx}>
